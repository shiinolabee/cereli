...
;(function($){
	...
	$.fn.scrollabletab = function(options){
		...
		return this.each(function(){
			...
			//Wrap inner items
			$tabs.wrap('<div id="stTabswrapper" class="stTabsMainWrapper" style="position:relative;"/>').find('.ui-tabs-nav').css('overflow','hidden').wrapInner('<div class="stTabsInnerWrapper" style="width:30000px"><span class="stWidthChecker" style=""/></div>');
			
			// var $widthChecker = $tabs.find('#tab-ul'),
			...
			$tabsWrapper.prepend(
			  $nav = $('<div/>')
			  		.disableSelection()
					.css({'position':'relative','z-index':3000})
					.append(
						$('<span/>')
							.disableSelection()
